/*
This test performs the DacDacScan with 832 sample pixels of the ROC, scanning through all columns and 16 pixels per column. Ntrig should be set at 1 otherwise the dataload is too large. 
-- author: Dehua Zhu
*/
...
#include <iostream>
#include "PixTestDacDacScanMultipix.hh"
#include "PixUtil.hh"
#include "log.h"
...
ClassImp(PixTestDacDacScanMultipix)

// ----------------------------------------------------------------------
PixTestDacDacScanMultipix::PixTestDacDacScanMultipix(PixSetup *a, std::string name) : 
PixTest(a, name), fParNtrig(-1), fParPHmap(-1), fParAllPixels(-1), fParDAC1("nada"), fParDAC2("nada"), 
  fParLoDAC1(-1), fParHiDAC1(-1), fParLoDAC2(-1), fParHiDAC2(-1) {
  PixTest::init();
  init(); 
  LOG(logDEBUG) << "PixTestDacDacScanMultipix ctor(PixSetup &a, string, TGTab *)";
}


//----------------------------------------------------------
PixTestDacDacScanMultipix::PixTestDacDacScanMultipix() : PixTest() {
  LOG(logDEBUG) << "PixTestDacDacScanMultipix ctor()";
}

// ----------------------------------------------------------------------
bool PixTestDacDacScanMultipix::setParameter(string parName, string sval) {
  bool found(false);
  string str1, str2; 
  string::size_type s1;
  int pixc, pixr; 
...
// ----------------------------------------------------------------------
void PixTestDacDacScanMultipix::init() {
  LOG(logDEBUG) << "PixTestDacDacScanMultipix::init()";
...
// ----------------------------------------------------------------------
void PixTestDacDacScanMultipix::setToolTips() {
...
// ----------------------------------------------------------------------
void PixTestDacDacScanMultipix::bookHist(string /*name*/) {
  fDirectory->cd(); 
...
//----------------------------------------------------------
PixTestDacDacScanMultipix::~PixTestDacDacScanMultipix() {
  LOG(logDEBUG) << "PixTestDacDacScanMultipix dtor";
}


// ----------------------------------------------------------------------
void PixTestDacDacScanMultipix::doTest() {
  ...
  LOG(logINFO) << "PixTestDacDacScanMultipix: " << dac1 << "[" << fParLoDAC1 << ", " << fParHiDAC1 << "]" 
 ...
 cout<<"performing second job."<<endl;
...
cout<<"performing third job."<<endl;
...
// ----------------------------------------------------------------------
TH1* PixTestDacDacScanMultipix::moduleMap(string histname) {
