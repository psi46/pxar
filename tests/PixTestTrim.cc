...
void PixTestTrim::trimTest() {
  ...
  // -- set trim bits
  int correction = 4;
  vector<TH1*> thr2  = scurveMaps("vcal", "TrimThr2", fParNtrig, 0, 199, 20, -1, 1); 
  ...
  vector<TH1*> thrF = scurveMaps("vcal", "TrimThrFinal", fParNtrig, fParVcal-20, fParVcal+20, 20, -1, 9); 
  PixTest::update(); 
...
void PixTestTrim::trimBitTest() {
  ...
  vector<TH1*> thr0 = mapsWithString(scurveMaps("Vcal", "TrimBitsThr0", fParNtrig, 0, 199, 20, -1, 7), "thr");
  ...
  for (unsigned int iv = 0; iv < vtrim.size(); ++iv) {
    ...
    thr = mapsWithString(scurveMaps("Vcal", Form("TrimThr_trim%d", btrim[iv]), fParNtrig, 0, static_cast<int>(maxThr)+10, 20, -1, 7), "thr");
