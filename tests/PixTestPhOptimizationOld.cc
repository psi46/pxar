...
ClassImp(PixTestPhOptimizationOld)


// ----------------------------------------------------------------------
PixTestPhOptimizationOld::PixTestPhOptimizationOld() {}


// ----------------------------------------------------------------------
PixTestPhOptimizationOld::PixTestPhOptimizationOld( PixSetup *a, string name ) :
  PixTest(a, name), fParNtrig(1), fSafetyMarginLow(20), fQuantMax(0.98), fVcalMax(100) {
  PixTest::init();
  init();
}
...
// ----------------------------------------------------------------------
bool PixTestPhOptimizationOld::setParameter(string parName, string sval) {
  bool found(false);
  string str1, str2;
  string::size_type s1;
  int pixc, pixr;
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::init() {
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::bookHist(string /*name*/) {}
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::doTest() {

  TStopwatch t;

  gStyle->SetPalette(1);
  cacheDacs();
  bigBanner(Form("PixTestPhOptimizationOld::doTest() Ntrig = %d", fParNtrig ));
  fDirectory->cd();
  PixTest::update();
...
//  }
  ...
  vector<pair<uint8_t, pair<int,int> > > badPixels;
  ...
  vector< pair<uint8_t, pair<uint8_t, vector<pxar::pixel> > > > dacdac_max;
  vector< pair<uint8_t, pair<uint8_t, vector<pxar::pixel> > > > dacdac_min;
  ...
  LOG(logINFO) << "PixTestPhOptimizationOld::doTest() done, duration: " << seconds << " seconds";
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::BlacklistPixels(vector<pair<uint8_t, pair<int, int> > > &badPixels, int aliveTrig){
  ...
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs();
  pair<uint8_t, pair<int, int> > badPix;
  Double_t eff=0.;
...
// ----------------------------------------------------------------------
pxar::pixel* PixTestPhOptimizationOld::RandomPixel(vector<pair<uint8_t, pair<int, int> > > &badPixels,
						uint8_t iroc) {
  ...
  do{
    ...
    isPixGood=true;
    for(vector<pair<uint8_t, pair<int, int> > >::iterator bad_it = badPixels.begin(); bad_it != badPixels.end(); bad_it++){
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::GetMaxPhPixel(map<int, pxar::pixel > &maxpixels,
					  vector<pair<uint8_t, pair<int,int> > >  &badPixels) {
  //looks for the pixel with the highest Ph at vcal = 255, taking care the pixels are not already saturating (ph=255)
  string rocType = fPixSetup->getConfigParameters()->getRocType();
  bool isRoc21 = true;
  if(!(rocType > "psi46digv2")){
    isRoc21 = false;
  }
  if(isRoc21){
    LOG(logINFO)<<"ROC type is newer than digv2";
  }
  LOG(logINFO)<<"ROC type is "<<rocType;
  fApi->_dut->testAllPixels(true);
  fApi->_dut->maskAllPixels(false);
  maskPixels();
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs();
  ...
  vector<pxar::pixel> result;
  vector<TH2D*> maxphmap;
  ...
  while((maxph>254 || maxph==0) && flag_maxPh<52){
    ...
    for (unsigned int ith2 = 0; ith2 < maxphmap.size(); ith2++) {
      ...
      for (vector<pair<uint8_t, pair<int, int> > >::iterator bad_it = badPixels.begin();
	  bad_it != badPixels.end(); bad_it++) {
	if (bad_it->second.first == colmax
	   && bad_it->second.second == rowmax
	   && bad_it->first == getIdFromIdx(ith2)) {
	  isPixGood=false;
	}
      }
...
//    else{
  ...
  map<uint8_t, pair<uint8_t, uint8_t> > maxpixs;
  pxar::pixel temp_pix;
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::GetMinPhPixel(map<int, pxar::pixel > &minpixels,
					  map<int, int> &minVcal,
					  vector<pair<uint8_t, pair<int,int> > >  &badPixels) {
...
//    else{
  ...
  map<uint8_t, pair<uint8_t, uint8_t> > minpixs;
  pxar::pixel temp_pix;
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::DrawPhMaps(map<int, int> &minVcal,
				       vector<pair<uint8_t, pair<int, int> > > &badPixels) {
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs();
  string name, title;
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::DrawPhCurves(map<int, pxar::pixel > &maxpixels,
					 map<int, pxar::pixel > &minpixels,
					 map<uint8_t, int> &po_opt,
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::MaxPhVsDacDac(vector<pair<uint8_t, pair<uint8_t, vector<pixel> > > > &dacdac_max,
					  map<int, pxar::pixel> maxpixels) {

  fApi->_dut->testAllPixels(false);
  fApi->_dut->maskAllPixels(true);
  for(map<int, pxar::pixel>::iterator maxp_it = maxpixels.begin(); maxp_it != maxpixels.end(); maxp_it++){
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::MinPhVsDacDac(vector<pair<uint8_t, pair<uint8_t, vector<pixel> > > > &dacdac_min,
					  map<int, pxar::pixel> minpixels,
					  map<int, int> &minVcal) {

  fApi->_dut->testAllPixels(false);
  fApi->_dut->maskAllPixels(true);
  for(map<int, pxar::pixel>::iterator minp_it = minpixels.begin(); minp_it != minpixels.end(); minp_it++){
  ...
  fApi->setVcalLowRange();
  for(map<int, int>::iterator ivcal = minVcal.begin(); ivcal != minVcal.end(); ivcal++){
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::SetMinThr(){
  fMinThr=0;
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::getTH2fromMaps(vector< pair<uint8_t, pair<uint8_t, vector<pxar::pixel> > > > &dacdac_max,
					   vector< pair<uint8_t, pair<uint8_t, vector<pxar::pixel> > > > &dacdac_min,
					   vector<TH2D* > &th2_max, vector<TH2D* > &th2_min) {
  ...
  vector< pair<uint8_t, pair<uint8_t, vector<pxar::pixel> > > >::iterator dacit_max = dacdac_max.begin();
  for(dacit_max = dacdac_max.begin(); dacit_max != dacdac_max.end(); dacit_max++){
    for(unsigned int ipix = 0; ipix< dacit_max->second.second.size(); ipix++){
      th2_max[getIdxFromId(dacit_max->second.second[ipix].roc())]->Fill(dacit_max->second.first,
									dacit_max->first,
									dacit_max->second.second[ipix].value());
    }
  }


  vector< pair<uint8_t, pair<uint8_t, vector<pxar::pixel> > > >::iterator dacit_min = dacdac_min.begin();
  for(dacit_min = dacdac_min.begin(); dacit_min != dacdac_min.end(); dacit_min++){
    for(unsigned int ipix = 0; ipix< dacit_min->second.second.size(); ipix++){
      th2_min[getIdxFromId(dacit_min->second.second[ipix].roc())]->Fill(dacit_min->second.first,
									dacit_min->first,
									dacit_min->second.second[ipix].value());
    }
  }
}
...
// ----------------------------------------------------------------------
void PixTestPhOptimizationOld::optimiseOnMapsNew(map<uint8_t, int> &po_opt,
					      map<uint8_t, int> &ps_opt,
					      vector<TH2D* > &th2_max,
					      vector<TH2D* > &th2_min,
					      vector<TH2D* > &th2_sol) {
  ...
  for(unsigned int iroc = 0; iroc < rocIds.size(); iroc++){
    ...
    for(int safetyCorrection = 0; maxsol<1. && safetyCorrection+fSafetyMarginLow<256; safetyCorrection++){
      LOG(logINFO)<< "safety margin for low PH: adding "<< safetyCorrection
		  << ", margin is now "<< safetyCorrection+fSafetyMarginLow;
      // hsol->Reset("M");
    ...
    if(maxsol < 1){
      LOG(logINFO) << "For ROC " << iroc
		   << ": No solutions with PhScale > 20 could be found. Looking for a solution with PhScale < 20.";
      for(int safetyCorrection = 0; maxsol<1. && safetyCorrection+fSafetyMarginLow<256; safetyCorrection++){
	LOG(logINFO) << "safety margin for low PH: adding "
		     << safetyCorrection
		     << ", margin is now "
		     << safetyCorrection+fSafetyMarginLow;
	// hsol->Reset("M");
	hmin = (TH2D*)th2_min[rocIds[iroc]]->Clone();
	hmax = (TH2D*)th2_max[rocIds[iroc]]->Clone();
	maxPH = hmax->GetMaximum();
