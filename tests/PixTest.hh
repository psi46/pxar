...
public:
  ...
  std::vector<std::vector<std::pair<int, int> > > deadPixels(int ntrig, bool scanCalDel = false, bool changeDACs = true);
  ...
  /// get the string describing anything (identified by what) in a test
  virtual std::string toolTip(std::string what);
