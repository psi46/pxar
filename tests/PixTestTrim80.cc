...
ClassImp(PixTestTrim80)

// ----------------------------------------------------------------------
PixTestTrim80::PixTestTrim80(PixSetup *a, std::string name) : PixTest(a, name), fParVcal(35), fParNtrig(1) {
  PixTest::init();
  init();
  //  LOG(logINFO) << "PixTestTrim80 ctor(PixSetup &a, string, TGTab *)";
}


//----------------------------------------------------------
PixTestTrim80::PixTestTrim80() : PixTest() {
  //  LOG(logINFO) << "PixTestTrim80 ctor()";
}

// ----------------------------------------------------------------------
bool PixTestTrim80::setParameter(string parName, string sval) {
  bool found(false);
...
// ----------------------------------------------------------------------
void PixTestTrim80::runCommand(std::string command) {
...
// ----------------------------------------------------------------------
void PixTestTrim80::setToolTips() {
...
// ----------------------------------------------------------------------
void PixTestTrim80::bookHist(string name) {
  fDirectory->cd();
...
//----------------------------------------------------------
PixTestTrim80::~PixTestTrim80() {
  LOG(logDEBUG) << "PixTestTrim80 dtor";
}


// ----------------------------------------------------------------------
void PixTestTrim80::doTest() {

  TStopwatch t;

  fDirectory->cd();
  PixTest::update();
  bigBanner(Form("PixTestTrim80::doTest()"));

  fProblem = false;
  trimTest();
  if (fProblem) {
    LOG(logINFO) << "PixTestTrim80::doTest() aborted because of problem ";
    return;
  }
  ...
  LOG(logINFO) << "PixTestTrim80::doTest() done, duration: " << seconds << " seconds";
}

// ----------------------------------------------------------------------
void PixTestTrim80::trimTest() {
  ...
  fDirectory->cd();
  PixTest::update();
  banner(Form("PixTestTrim80::trimTest() ntrig = %d, vcal = %d", fParNtrig, fParVcal));
  ...
  // -- summary printout
  LOG(logINFO) << "PixTestTrim80::trimTest() done";
...
// ----------------------------------------------------------------------
void PixTestTrim80::trimBitTest() {
  ...
  fDirectory->cd();
  PixTest::update();
  banner(Form("PixTestTrim80::trimBitTest() ntrig = %d, vtrims = %d %d %d %d",
  ...
  restoreDacs();
  setTrimBits();
  LOG(logINFO) << "PixTestTrim80::trimBitTest() done ";
  dutCalibrateOff();
}
...
// ----------------------------------------------------------------------
vector<TH1*> PixTestTrim80::trimStep(string name, int correction, vector<TH1*> calOld, int vcalMin, int vcalMax) {
...
// ----------------------------------------------------------------------
void PixTestTrim80::setTrimBits(int itrim) {
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs();
