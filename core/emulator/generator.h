...
namespace pxar {
  ...
  void fillRawData(uint32_t event, std::vector<uint16_t> &data, uint8_t tbm, uint8_t nrocs, bool empty, bool noise, size_t col, size_t row, std::vector<uint16_t> pattern = std::vector<uint16_t>(), uint32_t flags = 0);
